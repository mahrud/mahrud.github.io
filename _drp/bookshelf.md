---
layout: drp
title: Bookshelf
toc: "toc"
---

Thanks to a generous seed grant from the [DRP Network](https://www.drp-network.org),
we have purchased the following 22 books across a variety of topics. The books are
kept at the [Math Library](https://www.lib.umn.edu/math) and are available for those
looking for ideas to browse on top of the reference bookshelf. DRP participants can
borrow these for the entire duration of their participation.

<br />

Interested in donating books to the DRP Bookshelf? Please get in touch! ❤️

### Algebra:
- Groups and Symmetry, Armstrong
- An Introduction to Mathematical Cryptography, Hoffstein Pipher Silverman
- The Symmetric Group, Sagan
- Algebra, Lang

### Number Theory:
- The Arithmetic of Elliptic Curves, Joseph Silverman
- Elliptic Curves, Milne
- Rational Points on Elliptic Curves, Silverman, Tate
- Number Theory in Function Fields, Rosen

### Analysis:
- Fourier Analysis and Its Applications, Vretblad
- A Course in Functional Analysis, John Conway

### Applied Math:
- Ordinary Differential Equations with Applications, Carmen Chicone
- It's a Nonlinear World, Enns, Richard H.
- Financial Markets Theory, Barucci, Emilio, Fontana, Claudio
- An Introduction to Modeling Neuronal Dynamics, Börgers

### Topology and Geometry:
- Introduction to Smooth Manifolds, John Lee
- Differential Geometry, Loring Tu

### Combinatorics:
- Algebraic Combinatorics, Richard Stanley
- A Walk Through Combinatorics, Miklos Bona

### Algebraic and Combinatorial Geometry:
- Ideals, Varieties, and Algorithms, Cox Little O'Shea
- Combinatorial Commutative Algebra, Miller and Sturmfels

### Foundations:
- Category Theory for the Sciences, David Spivak
- Category Theory in Context, Emily Riehl
